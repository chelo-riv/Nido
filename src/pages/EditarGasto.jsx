...
export default function EditarGasto() {
  ...
  async function cargarDatos() {
    const [{ data: gasto }, { data: perfilesData }] = await Promise.all([
    ...
    if (gasto) {
      ...
      setPagadoPor(gasto.pagado_por)
      setFecha(gasto.fecha)
      setTipo(gasto.tipo ?? 'compartido')
      const pct = gasto.porcentaje_pagador ?? 50
      setPorcentaje(pct)
      const esPreset = PRESETS.some(p => p.valor === pct)
      if (!esPreset && gasto.tipo !== 'personal') {
        setModoCustom(true)
        setPorcentajeCustom(String(pct))
      }
    }
  ...
  return (
    <div className="min-h-screen bg-[#FAF7F4] pb-24 flex flex-col">
      ...
      <div className="bg-white border-b border-[#EDE8E3] px-4 pt-12 pb-4 flex items-center gap-3">
        ...
        <h1 className="text-lg font-bold text-[#2D2926]">Editar gasto</h1>
      </div>
      ...
      <div className="flex-1 px-4 pt-6 flex flex-col gap-5">
        ...
        <div className="bg-white rounded-2xl border border-[#EDE8E3] p-4">
          ...
          <div className="flex gap-2">
            {['compartido', 'personal'].map(t => (
              ...
              >
                {t === 'compartido' ? '🤝 Compartido' : '👤 Personal'}
              </button>
            ))}
          </div>
          {tipo === 'personal' && (
        ...
        {tipo === 'compartido' && (
          <div className="bg-white rounded-2xl border border-[#EDE8E3] p-4">
            ...
            <p className="text-xs text-[#8C7E75] mb-3">Pagador / El otro</p>
            ...
            {!modoCustom && (
              <div className="mt-3 bg-[#FAF7F4] rounded-xl px-4 py-2 flex justify-between">
                ...
                <span className="text-xs font-bold text-[#2D2926]">{100 - porcentaje}%</span>
              </div>
            )}
          </div>
        )}
        ...
        >
          {guardando ? 'Guardando...' : (
            <><Check size={18} strokeWidth={2.5} /> Guardar cambios</>
          )}
        </button>
